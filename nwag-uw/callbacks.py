...
def register_callbacks(dash_app):
    ...
    # Greeting callback to get current user name from Databricks Apps headers
    @dash_app.callback(
        Output("greeting-text", "children"),
        [Input("url", "pathname")]  # Trigger on page load
    )
    def update_greeting(pathname):
        """Update greeting with current user's name from Databricks Apps headers."""
        logger.info("Greeting callback triggered!")
        try:
            # In Databricks Apps, get user email from request headers
            from flask import request
            
            # Get user email from X-Forwarded-Email header (Databricks Apps standard)
            user_email = request.headers.get('X-Forwarded-Email')
            logger.info(f"X-Forwarded-Email header: {user_email}")
            ...
            if user_email:
                ...
                logger.info(f"Using env var username: {username}")
            ...
            logger.info(f"Final formatted username: {formatted_username}")
    ...
    # Document Summarization Callback
    @dash_app.callback(
        [Output("document-summary-content", "style"),
         Output("summarize-btn", "children")],
        Input("summarize-btn", "n_clicks"),
        State("document-summary-content", "style"),
        prevent_initial_call=True
    )
    def toggle_document_summary(n_clicks, current_style):
        """Toggle document summary visibility and update button text."""
        if n_clicks:
            ...
            return new_style, button_content
    ...
    def handle_upload_feedback(upload_contents, filenames):
        ...
        try:
            if upload_contents and filenames:
                ...
                failed_files = [f for f in uploaded_files if f['status'] == 'failed']
                ...
                if success_count == len(uploaded_files):
                    ...
                    for f in failed_files:
                        ...
                        error_details.append(f"{f['filename']}: {error_msg}")
                    
                    status_message = html.Div([
                        html.P(f"⚠️ {success_count}/{len(uploaded_files)} files uploaded successfully.", 
                               style={"fontWeight": "bold", "marginBottom": "10px"}),
                        html.Div([
                            html.P("Failed uploads:", style={"fontWeight": "600", "marginBottom": "5px"}),
                            html.Ul([html.Li(err, style={"fontSize": "12px", "color": "#d32f2f"}) for err in error_details])
                        ])
                    ])
    ...
    def parse_and_run_ai(n_clicks, contents, filenames):
        ...
        try:
            ...
            parse_query = f"""
            CREATE OR REPLACE TABLE richa_sethi.nwag.farmassessment_parsed AS
            WITH all_files AS (
              SELECT
                path,
                content
            ...
            try:
                ...
                # Set the catalog and schema first
                db_manager.execute_query("USE CATALOG richa_sethi")
                db_manager.execute_query("USE SCHEMA nwag")
                logger.info("Switched to richa_sethi.nwag catalog and schema")
                ...
                check_query = "SELECT COUNT(*) as count FROM richa_sethi.nwag.farmassessment_parsed"
                ...
                sample_query = "SELECT path, text FROM richa_sethi.nwag.farmassessment_parsed LIMIT 2"
                ...
                streaming_table = "richa_sethi.nwag.kie_86b9c0bd_endpoint_responses"
                ...
                WITH query_results AS (
                  ...
                  FROM richa_sethi.nwag.farmassessment_parsed
                )
                SELECT
                  input,
                ...
                features_table = "richa_sethi.nwag.farmassessment_features"
                features_query = f"""
                CREATE OR REPLACE TABLE {features_table} AS
                SELECT
                  get_json_object(cast(response AS STRING), '$.assessment_conducted_by') AS assessment_conducted_by,
                  get_json_object(cast(response AS STRING), '$.date') AS assessment_date,
                  
                  checklist_item.priority AS priority,
                  checklist_item.action_by.name AS action_by_name,
                  checklist_item.action_by.date AS action_by_date,
                  checklist_item.equipment_facility_location AS equipment_facility_location,
                  checklist_item.hazard_issue AS hazard_issue,
                  checklist_item.required_action AS required_action,
                  checklist_item.corrected AS corrected,

                  response AS features,
                  'AGR-4965' AS policy_number

                FROM {streaming_table}
                LATERAL VIEW EXPLODE(
                  from_json(
                    get_json_object(cast(response AS STRING), '$.checklist'),
                    'array<struct<
                        priority:string,
                        action_by:struct<date:string,name:string>,
                        equipment_facility_location:string,
                        hazard_issue:string,
                        corrected:string,
                        required_action:string
                      >>'
                  )
                ) exploded_checklist AS checklist_item
                WHERE error IS NULL
                """
                ...
                try:
                    ...
                    if features_data is not None and not features_data.empty:
                        ...
                        return f"✅ Success! Parsed {row_count} documents, extracted features with AI, and created structured farm assessment features table.", False
            ...
            return f"✅ {len(filenames)} file(s) has been uploaded to Databricks volumes and AI analysis completed! Parsed {row_count} documents, extracted features with AI, and created structured farm assessment features table.", False
        except Exception as e:
    ...
    def update_reference_specs_from_database(status_message):
        ...
        try:
            ...
            alter_query = """
            ALTER TABLE richa_sethi.nwag.farmassessment_features 
            ...
            SELECT 
                ...
                hazard_issue,
                required_action
            FROM richa_sethi.nwag.farmassessment_features 
            ...
            if result is not None and not result.empty:
                ...
                # Extract farm assessment fields
                priority = first_row.get('priority', 'No data available')
                priority_display = str(priority) if priority is not None and str(priority) != 'No data available' else "No data available"
                
                # Extract hazard issue
                hazard_issue = first_row.get('hazard_issue', 'No data available')
                hazard_display = str(hazard_issue) if hazard_issue is not None and str(hazard_issue) != 'No data available' else "No data available"
                
                # Extract required action
                required_action = first_row.get('required_action', 'No data available')
                action_display = str(required_action) if required_action is not None and str(required_action) != 'No data available' else "No data available"
                
                logger.info(f"Updated reference specs: {priority_display}, {hazard_display}, {action_display}")
                # Determine CSS classes based on data availability
                priority_class = "feature-value has-data" if priority_display != "No data available" else "feature-value placeholder-text"
                hazard_class = "feature-value has-data" if hazard_display != "No data available" else "feature-value placeholder-text"
                action_class = "feature-value has-data" if action_display != "No data available" else "feature-value placeholder-text"
                
                return (priority_display, hazard_display, action_display,
                        priority_class, hazard_class, action_class)
            else:
    ...
    @dash_app.callback(
        [Output("genie-messages", "children"),
         ...
         Output("genie-send-btn", "disabled")],
        [Input("genie-send-btn", "n_clicks"),
         Input("genie-input", "n_submit")],
        [State("genie-input", "value"),
         State("genie-messages", "children"),
         State("genie-conversation-id", "data")],
        prevent_initial_call=True
    )
    def handle_genie_message(n_clicks, n_submit, message, current_messages, conversation_id):
        """Handle Genie chat messages using Databricks Genie API."""
        logger.info(f"Genie callback triggered - n_clicks: {n_clicks}, n_submit: {n_submit}, message: {message}")
        
        if not message or (not n_clicks and not n_submit):
            logger.info("No message or no trigger, returning current state")
            return current_messages or [], "", conversation_id, False
        
        try:
            from genie_integration import get_genie_chat
            import pandas as pd
            
            genie = get_genie_chat()
            messages = current_messages or []
            
            # Remove welcome message if it exists
            if messages and len(messages) > 0:
                if isinstance(messages[0], dict) and 'props' in messages[0]:
                    children = messages[0].get('props', {}).get('children', [])
                    if isinstance(children, dict):
                        children_props = children.get('props', {}).get('children', [])
                        if isinstance(children_props, list) and len(children_props) > 0:
                            first_child = children_props[0]
                            if isinstance(first_child, dict) and 'props' in first_child:
                                class_name = first_child.get('props', {}).get('className', '')
                                if 'fa-robot' in str(class_name):
                                    messages = []
            
            # Add user message
            user_message = html.Div([
                html.Div([
                    html.Strong("You:", style={"color": "#333", "marginRight": "8px"}),
            ...
            if conversation_id:
                ...
                response = genie.continue_conversation(conversation_id, message)
            else:
                # Start new conversation
                response = genie.start_conversation(message)
            ...
            # Create response with SQL if available
            response_content = []
            
            # Add text response
            response_content.append(
                html.Div([
                    html.Strong("Genie:", style={"color": "#667eea", "marginRight": "8px"}),
                    html.Span(response_text.strip(), style={"color": "#333"})
            ...
            if response.get('sql'):
                response_content.append(
                    html.Div([
                        html.Strong("Generated SQL:", style={"fontSize": "12px", "color": "#666", "display": "block", "marginBottom": "5px"}),
                        html.Pre(
                            response.get('sql'),
                            style={
                                "background": "#f5f5f5",
            ...
            if response.get('data'):
                try:
                    ...
                    response_content.append(
                        html.Div([
                            html.Strong(f"Results ({len(df)} rows):", style={"fontSize": "12px", "color": "#666", "display": "block", "marginBottom": "5px"}),
                            html.Div([
                                html.Table([
                                    html.Thead(
                                        html.Tr([html.Th(col, style={"padding": "8px", "borderBottom": "2px solid #ddd", "fontSize": "12px"}) for col in df_preview.columns])
                                    ),
                                    html.Tbody([
                                        html.Tr([
                                            html.Td(str(df_preview.iloc[i][col]), style={"padding": "8px", "borderBottom": "1px solid #eee", "fontSize": "12px"})
                                            for col in df_preview.columns
                                        ]) for i in range(len(df_preview))
                                    ])
                                ], style={"width": "100%", "borderCollapse": "collapse", "fontSize": "12px"})
                            ], style={"overflowX": "auto", "maxHeight": "200px", "overflowY": "auto"})
                        ], style={"marginTop": "10px", "background": "#f9f9f9", "padding": "10px", "borderRadius": "4px"})
                    )
                except Exception as e:
            ...
            if response.get('error'):
                response_content.append(
                    html.Div([
                        html.I(className="fas fa-exclamation-triangle me-2", style={"color": "#dc3545"}),
                        html.Span(f"Error: {response.get('error')}", style={"color": "#dc3545", "fontSize": "12px"})
                    ], style={"marginTop": "10px", "padding": "8px", "background": "#fee", "borderRadius": "4px"})
                )
            
            genie_message = html.Div([
                html.Div(response_content, style={
                    "background": "#ffffff",
                    "padding": "12px 15px",
                    "borderRadius": "12px",
                    "marginBottom": "10px",
            ...
            messages.append(genie_message)
            
            logger.info("Genie callback completed successfully")
            return messages, "", conversation_id, False
            
        except Exception as e:
            logger.error(f"Error in Genie chat: {e}")
            import traceback
            traceback.print_exc()
            ...
            error_message = html.Div([
                html.Div([
                    html.I(className="fas fa-exclamation-triangle me-2", style={"color": "#dc3545"}),
                    html.Span(f"Sorry, I encountered an error: {str(e)}", style={"color": "#dc3545"})
                ], style={
                    "background": "#fee",
                    "padding": "12px 15px",
                    "borderRadius": "12px",
    ...
    # Clientside callback for auto-scrolling chat
    dash_app.clientside_callback(
        """
        function(children) {
            setTimeout(function() {
                var container = document.getElementById('genie-messages-scroll-container');
                if (container) {
        ...
        """,
        Output("genie-messages", "className"),
        Input("genie-messages", "children")
    )
    
    # Clientside callback to immediately show user message and clear input
    dash_app.clientside_callback(
        """
        function(n_clicks, n_submit, message, current_messages) {
            if ((n_clicks || n_submit) && message && message.trim()) {
                ...
                let messages = Array.isArray(current_messages) ? [...current_messages] : [];
                if (messages.length > 0 && messages[0].props && messages[0].props.children) {
                    const firstChild = messages[0].props.children;
                    if (firstChild && firstChild.props && firstChild.props.children) {
                        const hasRobot = JSON.stringify(firstChild).includes('fa-robot');
                        if (hasRobot) {
                            messages = [];
                        }
                    }
                ...
                messages.push(userMessageHTML);
                messages.push(loadingHTML);
        ...
        """,
        [Output("genie-messages", "children", allow_duplicate=True),
